...
watch(
  ...
  () => {
    if (materialRef.value) {
      const threeColor = new Color(props.color);
      materialRef.value.uniforms.uColor.value.set(threeColor.r, threeColor.g, threeColor.b);
      materialRef.value.uniforms.uFlakeSize.value = props.flakeSize;
      materialRef.value.uniforms.uMinFlakeSize.value = props.minFlakeSize;
      materialRef.value.uniforms.uPixelResolution.value = props.pixelResolution;
      materialRef.value.uniforms.uSpeed.value = props.speed;
      materialRef.value.uniforms.uDepthFade.value = props.depthFade;
      materialRef.value.uniforms.uFarPlane.value = props.farPlane;
      materialRef.value.uniforms.uBrightness.value = props.brightness;
      materialRef.value.uniforms.uGamma.value = props.gamma;
      materialRef.value.uniforms.uDensity.value = props.density;
      materialRef.value.uniforms.uVariant.value = props.variant === 'round' ? 1.0 : props.variant === 'snowflake' ? 2.0 : 0.0;
      materialRef.value.uniforms.uDirection.value = (props.direction * Math.PI) / 180;
    } else {
      cleanupFn?.();
      setupFn();
    }
...
<template>
  <div ref="containerRef" :class="['pixel-snow-container', className]" :style="style"></div>
</template>
...
.pixel-snow-container {
  ...
  width: 100vw;
  height: 100vh;
  pointer-events: none; /* Let clicks pass through if it's just a background */
}
</style>
